...
const ejecucionesRecientes = new Map();
function yaFueEjecutado(hora, toleranciaMin = 5) {
...
export function startCronJobs() {
    const ejecutarRecordatorio = async (hora) => {
        if (yaFueEjecutado(hora)) {
            console.log(`⚠️ Cron ya fue ejecutado recientemente para ${hora}, se omite.`);
            return;
        }
        try {
            console.log(`⏰ Ejecutando tarea programada (${hora}) - Enviando recordatorio`);
            const datos = await obtenerCampanaMesDíasPorRegistrarCron();
            console.log("🚀 ~ ejecutarRecordatorio ~ datos:", datos);
            if (datos.length === 0) {
                console.log("✅ No hay días faltantes por reportar, no se envía correo.");
                return;
            }
    ...
    const ejecutarReporteGerencia = async (hora) => {
        // if (yaFueEjecutado(hora)) {
        //   console.log(
        ...
        try {
            ...
            const datosUnicos = Array.from(new Map(datosEquipos.map((e) => [`${e.campana_nombre}-${e.equipo_nombre}`, e])).values());
            ...
            const safePercentage = (value) => !isNaN(value) ? `${value.toFixed(2)}%` : "0.00%";
            // 🔹 Calculamos porcentajes globales desde la BD
            const resumenPorcentual = await obtenerCampanaDiaReporteGerenciaPorcentualGeneral();
            console.log("🚀 ~ resumenPorcentual (desde BD) ~", resumenPorcentual);
            // 🔹 Enviar correo con los datos reales
            await sendReporteGerenciaEmail("patricia@2call.cl,romina@2call.cl,asistencia@2call.cl,marco@2call.cl,ana.valderrama@2call.cl,jorge.gomez@2callcenter.com,nelson.soto@kyros.cl,ignacio.fuentes@2call.cl,rita.montenegro@2call.cl,adrian@2call.cl,gonzalo.calderon@2call.cl,margarita@2call.cl,miguelangel@2call.cl,benjamin.arce@kyros.cl", new Date().toISOString().split("T")[0], datosUnicos, resumenPorcentual, // 👈 ahora sí usa los valores de la BD
            [], [], totales);
