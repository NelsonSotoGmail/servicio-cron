...
export type CampanaMesConEmails = {
    ...
    dias_faltantes: string[]; // fechas en ISO string
  };
  
  
  export async function obtenerCampanaMesDíasPorRegistrarCron(): Promise<CampanaMesConEmails[]> {
...
ORDER BY campana_nombre, turno_nombre, equipo_nombre, mes_orden;
  ...
  `;
    ...
    try {
      const { rows } = await pool.query<CampanaMesConEmails>(query);
      return rows;
    } catch (error) {
