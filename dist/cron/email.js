...
export async function sendGroupedReminderEmails(datos) {
    const yearActual = new Date().getFullYear();
    // Agrupamos por email
    const datosPorEmail = datos.reduce((acc, item) => {
              ...
              <table>
                ...
                <tbody>
                  <tr>
                    ...
                    <td>
                      <ul>
                        ${item.dias_faltantes.map(dia => `<li>${format(new Date(dia), 'dd-MM-yyyy', { locale: es })}</li>`).join('')}
                      </ul>
                    </td>
                  </tr>
                </tbody>
...
export async function sendReporteGerenciaEmail(destinatario, fechaReporte, datosEquipos, resumenPorcentual, estadisticasEquipos, detalleColaboradores, totales) {
    const yearActual = new Date().getFullYear();
    // 👉 Helper para formatear a porcentaje con 2 decimales
    const formatPorcentaje = (valor) => !isNaN(Number(valor)) ? `${Number(valor).toFixed(2)}%` : "0.00%";
    // 👉 HTML dinámico
    const html = `
<!DOCTYPE html>
<html lang="es">
